...
def venues():
    ...
    for area in areas:
        city = area.city
        state = area.state
        location_venues = Venue.query.filter_by(state=state).filter_by(city=city).all()
        venue = []
...
def search_venues():
    search_results = db.session.query(Venue).filter(Venue.name.ilike(f'%{request.form.get('search_term', '')}%')).all()
...
def delete_venue(venue_id):
    ...
    try:
        ...
        db.session.close()
        if error:
            flash('Error occurred: Venue ' + request.form['name'] + ' could not be deleted')
        else:
...
def search_artists():
    search_results = db.session.query(Artist).filter(Artist.name.ilike(f'%{request.form.get('search_term', '')}%')).all()
